...
def _relajar_cursos(con):
    ...
    SQLite no permite quitar un NOT NULL con ALTER, así que se reconstruye la
    tabla conservando todas las filas.
    """
    info = {f["name"]: f["notnull"] for f in con.execute("PRAGMA table_info(cursos)")}
    if not info.get("tipo_material"):
        return
    columnas = [c for c in
                ("id", "titulo", "descripcion", "categoria", "tipo_material", "carpeta",
                 "archivo", "archivo_original", "duracion_segundos", "paginas", "horas",
                 "vigencia_meses", "activo", "creado_en") if c in info]
    lista = ", ".join(columnas)

    con.commit()
    anterior = con.isolation_level
    con.isolation_level = None          # las PRAGMA no pueden ir dentro de una transacción
    try:
        con.execute("PRAGMA foreign_keys = OFF")
        con.execute("PRAGMA legacy_alter_table = ON")   # no reescribe las claves foráneas
        con.execute("ALTER TABLE cursos RENAME TO cursos_legacy")
        con.execute(CURSOS_NUEVO)
        con.execute(f"INSERT INTO cursos ({lista}) SELECT {lista} FROM cursos_legacy")
        con.execute("DROP TABLE cursos_legacy")
    finally:
...
def migrar(con):
    ...
    if "modulos" in tablas and "carpeta" in _columnas(con, "cursos"):
    ...
    # 2. La tabla de cursos deja de exigir el material (ahora es de los módulos).
    _relajar_cursos(con)

    # 3. Los quices pasan de colgar del curso a colgar del módulo.
    if "quices" in tablas and "curso_id" in _columnas(con, "quices"):
        con.execute("ALTER TABLE quices RENAME TO quices_old")
        ...
        con.execute(
            ...
            " FROM quices_old q JOIN modulos m ON m.curso_id = q.curso_id AND m.orden = 1")
        con.execute("DROP TABLE quices_old")

    # 4. Columnas nuevas en tablas que ya existían.
    if "intentos" in tablas and "modulo_id" not in _columnas(con, "intentos"):
        con.execute("ALTER TABLE intentos ADD COLUMN modulo_id INTEGER")
        con.execute(
            "UPDATE intentos SET modulo_id = ("
            "  SELECT q.modulo_id FROM quices q WHERE q.id = intentos.quiz_id)")
    if "certificados" in tablas and "origen" not in _columnas(con, "certificados"):
        con.execute("ALTER TABLE certificados ADD COLUMN origen TEXT NOT NULL DEFAULT 'plataforma'")

    # 5. El avance por curso se replica al módulo 1, para no perder progreso.
    if "avance_modulos" in tablas and "asignaciones" in tablas:
        filas = con.execute(
