...
## Caso concreto: Hostinger

Sirve, pero con un plan **VPS (KVM)**, no con el hosting web compartido: ese solo ejecuta PHP y no
permite instalar Flask ni dejar la aplicación corriendo. El dominio sí puedes comprarlo y
administrarlo en Hostinger igual; lo único que cambia es dónde vive la aplicación.

Plan recomendado: **KVM 2** (2 vCPU, 8 GB de RAM, 100 GB NVMe). El KVM 1 funciona, pero convertir
un `.pptx` con LibreOffice consume RAM en ráfagas y los videos llenan rápido los 50 GB.

1. Compra el VPS. Al crearlo, en el sistema operativo elige la plantilla **Ubuntu 24.04 con Docker**
   (hPanel la instala con un clic) y guarda la contraseña de root.
2. En hPanel → **Dominios → Zona DNS**, crea un registro **A**:
   nombre `capacitacion`, valor = la IP del VPS. Queda `capacitacion.tudominio.com` y tu sitio
   principal sigue intacto donde está.
3. Conéctate por SSH o por el **terminal del navegador** que trae hPanel, y sube el proyecto:

```bash
apt update && apt install -y git
git clone TU_REPOSITORIO capacitacion    # o sube el .zip por SFTP y descomprímelo
cd capacitacion
nano docker-compose.yml                  # cambia DOMINIO, CORREO y SECRET_KEY
docker compose up -d --build
docker compose exec app python crear_admin.py
```

4. Abre `https://capacitacion.tudominio.com`. El certificado lo emite y renueva Caddy solo.

Un detalle del firewall: en hPanel → **VPS → Firewall**, deja abiertos los puertos 80 y 443. Si no,
la validación del certificado falla y el sitio no carga.

Para los respaldos, Hostinger incluye snapshots semanales del VPS, pero conviene además sacar copia
de la carpeta de datos con el comando `tar` de la opción B: pesa poco y la recuperas en cualquier
servidor.
