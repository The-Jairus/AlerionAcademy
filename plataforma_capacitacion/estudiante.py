...
def modulo(asignacion_id, modulo_id):
    ...
    return render_template("estudiante/modulo.html", a=asignacion, bloque=actual,
                           ...
                           video_src=url_video(asignacion_id, actual["m"]),
                           minimo_video=config.PORCENTAJE_MIN_VIDEO,
                           segundos_pagina=config.SEGUNDOS_MIN_POR_PAGINA)


def url_video(asignacion_id, modulo):
    """URL del video: la sirve Caddy si está activado, si no la aplicación."""
    if config.MEDIA_DIRECTO and modulo["carpeta"] and modulo["archivo"]:
        return f"{config.MEDIA_PREFIJO}/materiales/{modulo['carpeta']}/{modulo['archivo']}"
    return url_for("estudiante.video", asignacion_id=asignacion_id, modulo_id=modulo["id"])
...
def autorizar_medio():
    """Caddy pregunta aquí antes de entregar un archivo de material.

    Responde 204 si la persona puede verlo y 403 si no. La comprobación es
    instantánea, así que el proceso queda libre de inmediato: la entrega del
    archivo, que es lo lento, la hace Caddy.
    """
    usuario = usuario_actual()
    if usuario is None:
        return "", 401

    ruta = (request.headers.get("X-Forwarded-Uri") or request.args.get("uri") or "")
    partes = [p for p in ruta.split("?")[0].strip("/").split("/") if p]
    # se espera media/materiales/<carpeta>/<archivo>
    if len(partes) < 4 or partes[0] != config.MEDIA_PREFIJO.strip("/") or partes[1] != "materiales":
        return "", 403
    carpeta = partes[2]

    if usuario["rol"] == "admin":
        permitido = consultar("SELECT 1 AS ok FROM modulos WHERE carpeta = ? LIMIT 1",
                              (carpeta,), uno=True)
    else:
        permitido = consultar(
            "SELECT 1 AS ok FROM modulos m JOIN asignaciones a ON a.curso_id = m.curso_id"
            " JOIN cursos c ON c.id = m.curso_id"
            " WHERE m.carpeta = ? AND a.usuario_id = ? AND c.activo = 1 LIMIT 1",
            (carpeta, usuario["id"]), uno=True)
    return ("", 204) if permitido else ("", 403)
