...
def crear_usuario():
    ...
    if not nombre or not email or len(clave) < 8:
        flash("Nombre, correo y una contraseña de mínimo 8 caracteres son obligatorios.", "error")
...
@bp.post("/usuarios/<int:usuario_id>/editar")
@admin_requerido
def editar_usuario(usuario_id):
    ...
    flash(f"Datos de {nombre} actualizados.", "ok")
    return _volver("admin.usuarios")
...
def cambiar_estado_usuario(usuario_id):
    ...
    flash(f"{usuario['nombre']} quedó {'activo' if nuevo else 'inactivo'}.", "ok")
    return _volver("admin.usuarios")
...
def eliminar_usuario(usuario_id):
    ...
    flash(f"{usuario['nombre']} y su historial fueron eliminados.", "ok")
    return _volver("admin.usuarios")
...
def crear_curso():
    ...
    try:
        ...
        flash(f"No se pudo procesar el material: {error}", "error")
        return _volver("admin.cursos")
    ...
    if info["tipo_material"] == "documento":
        detalle = f"{info['paginas']} páginas"
    else:
        detalle = f"{info['duracion_segundos'] / 60:.0f} minutos de video"
    flash(f"Curso «{titulo}» creado con {detalle}. Ahora crea su examen.", "ok")
    return redirect(url_for("admin.editar_quiz", curso_id=curso_id))


@bp.post("/cursos/<int:curso_id>/editar")
@admin_requerido
def editar_curso(curso_id):
    ...
    flash("Curso actualizado.", "ok")
    return _volver("admin.cursos")


@bp.post("/cursos/<int:curso_id>/estado")
@admin_requerido
def cambiar_estado_curso(curso_id):
    ...
    flash("Visibilidad del curso actualizada.", "ok")
    return _volver("admin.cursos")


@bp.post("/cursos/<int:curso_id>/eliminar")
@admin_requerido
def eliminar_curso(curso_id):
    ...
    flash(f"Curso «{curso['titulo']}» eliminado.", "ok")
    return _volver("admin.cursos")
...
def crear_asignacion():
    ...
    if not curso_ids or not usuario_ids:
        flash("Selecciona al menos un curso y un usuario.", "error")
    ...
    mensaje = f"{nuevas} asignación(es) creada(s)."
    if repetidas:
        mensaje += f" {repetidas} ya existían y se dejaron intactas."
    flash(mensaje, "ok")
    return _volver("admin.asignaciones")
...
def _notificar_asignacion(curso_ids, usuario_ids, fecha_limite):
    ...
    for usuario_id in usuario_ids:
        ...
        if not usuario:
            continue
        cuerpo = (f"Hola {usuario['nombre'].split()[0]},\n\n"
                  "Te asignamos la siguiente formación en la plataforma de capacitación:\n"
                  + "\n".join(f"  · {t}" for t in titulos)
                  + (f"\n\nFecha límite: {fecha_limite}" if fecha_limite else "")
                  + f"\n\n{ajustes.get('org_nombre', '')} · {ajustes.get('org_area', '')}")
        mailer.enviar(ajustes, usuario["email"], "Nueva capacitación asignada", cuerpo)


@bp.post("/asignaciones/<int:asignacion_id>/eliminar")
@admin_requerido
def eliminar_asignacion(asignacion_id):
    ejecutar("DELETE FROM asignaciones WHERE id = ?", (asignacion_id,))
    flash("Asignación retirada.", "ok")
    return redirect(request.referrer or url_for("admin.asignaciones"))


@bp.post("/asignaciones/<int:asignacion_id>/reabrir")
@admin_requerido
def reabrir_asignacion(asignacion_id):
    ...
    flash("Capacitación reabierta: el usuario debe verla y presentar el examen de nuevo.", "ok")
    return redirect(request.referrer or url_for("admin.asignaciones"))
...
@bp.post("/quices/<int:curso_id>/guardar")
@admin_requerido
def guardar_quiz(curso_id):
    titulo = (request.form.get("titulo") or "").strip() or "Examen del curso"
    try:
    ...
    flash("Examen guardado.", "ok")
    return redirect(url_for("admin.editar_quiz", curso_id=curso_id))


@bp.post("/quices/<int:curso_id>/preguntas")
@admin_requerido
def agregar_pregunta(curso_id):
    quiz = consultar("SELECT * FROM quices WHERE curso_id = ?", (curso_id,), uno=True)
    if quiz is None:
        flash("Primero guarda la configuración del examen.", "error")
        return redirect(url_for("admin.editar_quiz", curso_id=curso_id))
    ...
    if not enunciado or len(opciones_validas) < 2:
        flash("Escribe la pregunta y al menos dos opciones de respuesta.", "error")
        return redirect(url_for("admin.editar_quiz", curso_id=curso_id))
    if correcta not in {str(i) for i, _ in opciones_validas}:
        flash("Marca cuál de las opciones escritas es la correcta.", "error")
        return redirect(url_for("admin.editar_quiz", curso_id=curso_id))
    ...
    flash("Pregunta agregada.", "ok")
    return redirect(url_for("admin.editar_quiz", curso_id=curso_id))


@bp.post("/preguntas/<int:pregunta_id>/eliminar")
@admin_requerido
...
@bp.post("/certificados/<codigo>/reenviar")
@admin_requerido
def reenviar_certificado(codigo):
    ...
    asunto = f"Certificado de capacitación · {curso['titulo']}"
    destinos = [usuario["email"]]
    if ajustes.get("correo_administrador"):
        destinos.append(ajustes["correo_administrador"])
    mailer.enviar(ajustes, destinos, asunto,
                  mailer.cuerpo_certificado(usuario, curso, certificado["puntaje"], codigo, ajustes),
                  ruta)
    flash(f"Certificado {codigo} reenviado a {', '.join(destinos)}.", "ok")
...
@bp.post("/configuracion")
@admin_requerido
def guardar_configuracion():
    campos = ["org_nombre", "org_area", "org_firmante", "org_cargo_firmante",
              "correo_administrador", "smtp_host", "smtp_puerto", "smtp_usuario",
              "smtp_remitente"]
...
@bp.post("/configuracion/probar-correo")
@admin_requerido
def probar_correo():
    ajustes = obtener_config()
    destino = (request.form.get("destino") or ajustes.get("correo_administrador") or "").strip()
    if not destino:
        flash("Escribe un correo de destino para la prueba.", "error")
        return _volver("admin.configuracion")
    mailer.enviar(ajustes, destino, "Prueba de correo · plataforma de capacitación",
                  "Si recibes este mensaje, el envío de certificados va a funcionar.",
                  en_segundo_plano=False)
    flash(f"Prueba enviada a {destino}. Revisa el registro de abajo para ver el resultado.", "ok")
    return _volver("admin.configuracion")


@bp.route("/certificado/<codigo>.pdf")
@admin_requerido
