...
def login():
    ...
    if request.method == "POST":
        ...
        if espera:
            flash(f"Demasiados intentos fallidos. Intenta de nuevo en {espera // 60 + 1} minutos.",
                  "error")
        ...
        if usuario and not usuario["activo"]:
            flash("Tu usuario está inactivo. Comunícate con el administrador.", "error")
        else:
...
@bp.route("/mi-clave", methods=["POST"])
@login_requerido
def cambiar_clave():
