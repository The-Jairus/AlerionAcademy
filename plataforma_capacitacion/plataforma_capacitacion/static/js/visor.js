  ...
  if (imagen) {
    ...
    function refrescarSiguiente() {
      ...
      if (actual >= C.paginas) {
        ...
        siguiente.textContent = completo ? 'Contenido completo' : 'Última página';
        if (completo && mensaje) mensaje.textContent = 'Terminaste el contenido: ya puedes presentar el examen.';
        return;
      }
      siguiente.textContent = 'Siguiente →';
      if (actual < maxAlcanzada) {          // ya la leyó antes
        siguiente.disabled = false;
        if (mensaje) mensaje.textContent = 'Puedes moverte libremente por las páginas que ya leíste.';
        return;
      }
      ...
      const tic = () => {
        if (restante <= 0) {
          clearInterval(cuentaRegresiva);
          siguiente.disabled = false;
          if (mensaje) mensaje.textContent = 'Puedes pasar a la siguiente página.';
          return;
        }
        if (mensaje) mensaje.textContent = 'Podrás avanzar en ' + restante + ' s.';
        restante -= 1;
      };
      tic();
