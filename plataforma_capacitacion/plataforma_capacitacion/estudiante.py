...
@bp.route("/curso/<int:asignacion_id>")
@login_requerido
...
@bp.route("/material/<int:asignacion_id>/pagina/<int:numero>.png")
@login_requerido
...
@bp.post("/api/avance/<int:asignacion_id>")
@login_requerido
def avance(asignacion_id):
    ...
    if asignacion["tipo_material"] == "video":
        ...
        if total <= 0:
            return jsonify(error="El curso no tiene duración registrada."), 400
        if posicion > maximo + config.SALTO_MAX_SEGUNDOS:
            return jsonify(avance=round(100 * maximo / total, 2), max_posicion=maximo,
                           habilitado=completo,
                           mensaje="Debes ver el video en orden; no se registran los saltos."), 200
        ...
        if total_paginas <= 0:
            return jsonify(error="El curso no tiene páginas registradas."), 400
        if pagina_actual > maximo + 1:
            return jsonify(mensaje="Avanza página por página."), 200
        ...
        if pagina_actual > maximo:
            ...
            if time.time() - ultimo < config.SEGUNDOS_MIN_POR_PAGINA - 1:
                return jsonify(avance=round(100 * maximo / total_paginas, 2),
                               ...
                               mensaje="Tómate unos segundos para leer la página."), 200
...
# --------------------------------------------------------------------------- examen
@bp.route("/curso/<int:asignacion_id>/examen", methods=["GET", "POST"])
@login_requerido
def examen(asignacion_id):
    ...
    if not asignacion["quiz_id"]:
        flash("Este curso todavía no tiene examen configurado.", "error")
        return redirect(url_for("estudiante.curso", asignacion_id=asignacion_id))
    if not asignacion["material_completo"]:
        flash("Termina primero el contenido del curso para presentar el examen.", "error")
        return redirect(url_for("estudiante.curso", asignacion_id=asignacion_id))
    if asignacion["estado"] == "aprobado":
        flash("Ya aprobaste este curso.", "ok")
        return redirect(url_for("estudiante.curso", asignacion_id=asignacion_id))

    usados = _intentos_usados(asignacion_id)
    if usados >= asignacion["intentos_max"]:
        flash("Agotaste los intentos disponibles. Solicita una reapertura al administrador.",
              "error")
        return redirect(url_for("estudiante.curso", asignacion_id=asignacion_id))

    preguntas = _preguntas_del_quiz(asignacion["quiz_id"])
    if not preguntas:
        flash("El examen aún no tiene preguntas cargadas.", "error")
        return redirect(url_for("estudiante.curso", asignacion_id=asignacion_id))
...
def emitir_certificado(asignacion, intento_id, puntaje) -> str:
    ...
    asunto = f"Certificado de capacitación · {curso_['titulo']}"
    mailer.enviar(ajustes, usuario["email"], asunto,
                  mailer.cuerpo_certificado(usuario, curso_, puntaje, codigo, ajustes), ruta)
    admin_email = ajustes.get("correo_administrador")
    if admin_email:
        mailer.enviar(ajustes, admin_email, f"[Copia] {asunto}",
