...
def _crear_curso_documento(con):
    ...
    return con.execute(
        ...
        ("Ramp Safety Fundamentals",
         "Danger zones, PPE, ground vehicle movement and FOD prevention on the ramp.",
         "Safety", carpeta_id, pdf.name,
         "ramp-safety.pdf", paginas, ahora())).lastrowid
...
def _crear_curso_video(con):
    ...
    return con.execute(
        ...
        ("New Hire Onboarding",
         ...
         "Onboarding", carpeta_id, destino.name, "onboarding.mp4", duracion, ahora())).lastrowid
...
def sembrar(reiniciar=False):
    ...
    _crear_quiz(con, curso_doc, "Ramp Safety Assessment", PREGUNTAS_RAMPA, minimo=80)
    if curso_video:
        _crear_quiz(con, curso_video, "Onboarding Assessment", PREGUNTAS_INDUCCION, minimo=70)
